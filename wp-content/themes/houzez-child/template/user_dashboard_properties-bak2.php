                            ...
                            <?php if ($_SERVER['REMOTE_ADDR'] == '114.109.15.179') { ?>
                              ...
                              if (count($_posts) > 0) {
                                ...
                                foreach ($_posts as $index => $_post) {
                                  $_post_permalink = str_ireplace(get_option('home'), '', get_post_permalink($_post));
                                  $_post_permalink = trim($_post_permalink, '/');
                                  $_post_permalink = '/' . $_post_permalink . '/';
                                  $landingPagePath = 'ga:landingPagePath=='. rawurlencode($_post_permalink) .'';
                                  // $landingPagePath[] = 'ga:landingPagePath=='. rawurlencode($_post_permalink) .'';

                                  $options = [
                                    'dimensions' => 'ga:landingPagePath',
                                    'sort' => '-ga:pageviews',
                                    'filters' => $landingPagePath,
                                  ];
                                  $results = getResults($analytics, $params, $options);
                                  $print = printResults($results);

                                  echo '<pre>'; print_r($print); echo '</pre>';
                                }

                                // $landingPagePath = implode(',', $landingPagePath);
                                // echo '<pre>'; print_r($landingPagePath); echo '</pre>';
                                ...
                                //   'dimensions' => 'ga:landingPagePath',
                                //   'sort' => '-ga:pageviews',
                                //   'filters' => 'ga:landingPagePath=~/property/('. $landingPagePath .')/$',
                                // ];
                                // $results = getResults($analytics, $params, $options);
                                // $print = printResults($results);
                                //
                                ...
                                echo '<pre>'; print_r('<h4>'.$houzez_local['properties_not_found'].'</h4>'); echo '</pre>';
                              }
                              ?>
                            <?php } ?>
