...
class LocationController extends Controller
{
    ...
    public function stateList(){
        ...
        $states = State::select('states.id', 'state_name', 'country_name', 'country_id')->leftJoin('countries', 'countries.id','=','states.country_id')->paginate(50);

        if ($searchTerm){
            $states = State::select('states.id', 'state_name', 'country_name', 'country_id')->where('state_name', 'like', "%$searchTerm%")->leftJoin('countries', 'countries.id','=','states.country_id')->paginate(50);
        }
    ...
    public function cityList(){
        ...
        $searchTerm = \request('q');
        if ($searchTerm){
            $cities = City::with('state')->where('city_name', 'like', "%$searchTerm%")->paginate(20);
        }else{
            $cities = City::with('state')->paginate(20);
        }
